...
function phanTich_URL_chiTietSanPham() {
    ...
    // Đổi title
    document.title = nameProduct + ' - Thế giới Cafe';
    ...
    var s = addThongSo('Thành Phần', sanPhamHienTai.detail.screen);
    s += addThongSo('Chỉ Tiêu Chất Lượng', sanPhamHienTai.detail.os);
    s += addThongSo('Khẩu vị', sanPhamHienTai.detail.camara);
    s += addThongSo('Cỡ Hạt', sanPhamHienTai.detail.camaraFront);
    s += addThongSo('Thành Phần', sanPhamHienTai.detail.cpu);
    s += addThongSo('Định Lượng', sanPhamHienTai.detail.ram);
    s += addThongSo('Ngày Sản Xuất', sanPhamHienTai.detail.rom);
    s += addThongSo('Hạn Sử Dụng', sanPhamHienTai.detail.microUSB);
    s += addThongSo('Chứng Nhận An Toàn Thực Phẩm Số: ', sanPhamHienTai.detail.battery);
    info.innerHTML = s;
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        case 'muanhieunhat':
            ...
            return `Khách hàng mua 3 có thể tặng 1 sản phẩm với ` + span + `với thời hạn 1 tháng kể từ khi mua hàng.`;
